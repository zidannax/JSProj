describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ JSProj }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(JSProj.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(JSProj.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ JSProj }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      JSProj.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ JSProj }) => {
      ...
      const { files } = JSProj.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      JSProj.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ JSProj }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(JSProj.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ JSProj }) => {
      const { files } = JSProj.getState()
      JSProj.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        JSProjserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'JSProj-core:3.2.0,JSProj-transloadit:3.1.3,JSProj-tus:3.1.0,JSProj-dropbox:3.1.1,JSProj-box:2.1.1,JSProj-facebook:3.1.1,JSProj-google-drive:3.1.1,JSProj-instagram:3.1.1,JSProj-onedrive:3.1.1,JSProj-zoom:2.1.1,JSProj-url:3.3.1',
    ...
    cy.get('.JSProj-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.JSProj-StatusBar-actions > .JSProj-c-btn').click()
    cy.wait([
