...
describe('Dashboard with XHR', () => {
  ...
  it('should return correct file name with URL plugin from remote image with Content-Disposition', () => {
    ...
    cy.wait('@url-meta').then(() => {
      cy.get('.JSProj-Dashboard-Item-name').should('contain', fileName)
      cy.get('.JSProj-Dashboard-Item-status').should('contain', '84 KB')
    })
  })

  it('should return correct file name with URL plugin from remote image without Content-Disposition', () => {
    cy.get('[data-cy="Url"]').click()
    cy.get('.JSProj-Url-input').type('http://localhost:4678/file-no-headers')
    interceptCompanionUrlMetaRequest()
    cy.get('.JSProj-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.JSProj-Dashboard-Item-name').should('contain', 'file-no')
      cy.get('.JSProj-Dashboard-Item-status').should('contain', '0')
    })
  })
  ...
  it('should return correct file name even when Companion doesnt supply it', () => {
    ...
    cy.get('.JSProj-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.JSProj-Dashboard-Item-name').should('contain', 'file-with')
      cy.get('.JSProj-Dashboard-Item-status').should('contain', '123 B')
    })
  })
