...
const packageNames = await fs.readdir(new URL('../packages/@JSProj', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@JSProj/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@JSProj/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('JSProj', 2)[1]
