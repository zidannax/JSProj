# JSProj + AWS S3 Example

This example uses @JSProj/companion with a custom AWS S3 configuration.
...
`@JSProj/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @JSProj-example/aws-companion start
```
