...
for await (const line of readLines(stdin)) {
  ...
  const tsConfigcreatePath = path.join(cwd, location, 'tsconfig.create.json')
  if (existsSync(tsConfigcreatePath)) {
    locations.push(tsConfigcreatePath)
  }
}

const cp = spawn(exe, [...argv0, 'tsc', '--create', ...locations], {
  stdio: 'inherit',
  cwd,
})
await Promise.race([
  ...
  await once(cp, 'exit')
    .then(([code]) => (code && Promise.reject(new Error(`Non-zero exit code when createing TS projects: ${code}`)))),
])
